...
(transacted-only DMs, tournament capacity), `security` (response-privacy sweep across the API,
malformed-id handling, stranger-vs-order authorization), `hardening` (upload validation + served
headers, CORS/error hygiene, helmet, `/health`, Maintenance Mode, authz spot checks). BOG callbacks with real signatures are NOT covered (no
