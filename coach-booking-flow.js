(function () {
  ...
  const icons = {
    ...
    dollar: '<text x="12" y="17" text-anchor="middle" fill="currentColor" stroke="none" font-size="17" font-weight="700">₾</text>',
  ...
  function renderReview() {
    ...
    return `${intro('STEP 4 OF 6', 'Review Your Booking', 'Please review all details before proceeding to payment.')}${coachCard()}<div class="booking-review-list">
      ...
      <article><span class="green">${icon('dollar')}</span><div><h2>Total Price</h2><strong>${selectedPackage().id === 'single' ? 'One-time session · No subscription' : `Includes all ${selectedPackage().sessions} sessions`}</strong></div><b class="booking-total">${money(price())}</b></article>
  ...
  function renderPayment() {
    return `${intro('STEP 5 OF 6', 'Payment', 'Complete your payment to secure your coaching session.')}${coachCard()}
      ...
      <article class="booking-payment-total"><span class="green">${icon('dollar')}</span><div><h2>Total Price</h2><small>${selectedPackage().id === 'single' ? 'One-time session · No subscription' : `Includes all ${selectedPackage().sessions} sessions`}</small></div><b>${money(price())}</b></article>${navButtons('Pay Now', { lock: true, note: "Secure payment  •  You'll return to WaveHub after payment" })}`;
  }
